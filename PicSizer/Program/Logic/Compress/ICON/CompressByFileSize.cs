    ...
    public partial class ICON
    {
        ...
        public void Compress_By_FileSize(long limit, string output)
        {
            ...
            while (left < right - 1)
            {
                ...
                mid = (left + right) / 2;
                //获取当前画质下的图片大小,并存入数组
                sizeList[mid] = GetBitmapSize(ref bitmap, mid);
                //根据大小往左或往右查找
            ...
            if (sizeList[left] == 0)
            {
                sizeList[left] = GetBitmapSize(ref bitmap, left);
            }
            //如果文件大小符合要求或者接受超出限制的文件就输出
            if (sizeList[left] <= (limit - 1) * 1024 || PicSetting.AcceptExceedPicture)
            {
                _CompressByCPP(ref bitmap, left);
                SaveIconToFile(ref bitmap, output);
            }
            else
            {
        ...
        /// </summary>
        public static long GetBitmapSize(ref Bitmap bitmap, int value)
        {
            ...
            try
            {
                ...
                _CompressByCPP(ref bitmap, value);
                //此处忽略ICON标头大小
